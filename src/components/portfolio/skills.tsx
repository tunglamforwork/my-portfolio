...
export function Skills() {
	return (
		<section id='skills' className='py-24 px-6 bg-[#F5F3EE]'>
				...
				<div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6'>
					{CATEGORIES.map((cat, i) => (
						<FadeIn key={cat.name} delay={i * 0.09}>
							<div className='border-[3px] border-[#0B0B0B] rounded-[24px] shadow-[6px_6px_0_#0B0B0B] bg-white h-full overflow-hidden transition-all duration-[250ms] hover:-translate-x-1 hover:-translate-y-1'>
								...
								<div className='p-6'>
									...
									<div className='flex flex-wrap gap-2'>
										{cat.items.map((skill) => (
											<span
												key={skill}
												className='text-[12px] font-[700] px-3 py-1.5 border-[2px] border-[#0B0B0B] rounded-full'
